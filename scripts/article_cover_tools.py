...
def audit() -> dict:
    ...
    for row in rows:
        ...
        missing_meta = [key for key in REQUIRED_SOCIAL_META if not meta_content(html, key)]
        if missing_meta:
            social_meta_missing.append({"file": row["file"], "missing": missing_meta})
        og_width = meta_content(html, "og:image:width")
        og_height = meta_content(html, "og:image:height")
        if og_width != str(WIDTH) or og_height != str(HEIGHT):
            social_meta_wrong_size.append({
                "file": row["file"],
                "width": og_width,
                "height": og_height,
            })
    ...
    return {
        ...
        "socialMetaMissing": len(social_meta_missing),
        "socialMetaWrongSize": len(social_meta_wrong_size),
